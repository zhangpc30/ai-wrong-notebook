...
test('front-end contains no model secret or direct provider endpoint', () => {
  ...
  assert.equal(content.includes('wx.cloud.callContainer'), true)
  assert.equal(content.includes('wx.cloud.uploadFile'), true)
  assert.equal(content.includes('wx.cloud.getTempFileURL'), true)
  assert.equal(content.includes('wx.cloud.deleteFile'), true)
  assert.equal(content.includes("'X-WX-SERVICE': CLOUD_SERVICE"), true)
...
test('MVP pages expose required configuration, capture and review controls', () => {
  ...
  assert.match(read('pages/home/index.wxml'), /AI 云服务尚未就绪/)
  assert.match(read('pages/capture/index.wxml'), /bindtap="chooseFromCamera"/)
  assert.match(read('pages/capture/index.wxml'), /裁剪聚焦/)
  assert.match(read('pages/capture/index.js'), /wx\.chooseImage/)
  assert.match(read('pages/capture/index.js'), /wx\.editImage/)
  assert.match(read('pages/capture/index.js'), /wx\.cropImage/)
  assert.match(read('app.json'), /用于拍摄需要 AI 分析的错题图片/)
  ...
  assert.match(read('pages/settings/index.wxml'), /微信云托管/)
  ...
  assert.match(read('utils/cloud.js'), /prod-d5gzlt33v072ebddc/)
  assert.match(read('utils/cloud.js'), /express-vm2i/)
